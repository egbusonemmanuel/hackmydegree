...
// Use unicode escapes to avoid PowerShell parsing issues
const GRAD   = '\u{1F393}';
const MONEY  = '\u{1F4B0}';
const NOTES  = '\u{1F4DD}';
const BOOK   = '\u{1F4D6}';
...
const out = path.join(__dirname, '..', 'src', 'pages', 'WelcomePage.jsx');

const code = `// src/pages/WelcomePage.jsx
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { SparklesIcon } from '../components/SharedUI';

const FA = [
  { icon: '${NOTES}',  title: 'Verified Past Questions',     desc: "Model answers mapped to your university grading rubric so you know exactly what examiners want." },
  { icon: '${BOOK}',   title: 'Lecture Notes & Study Atlas', desc: 'Interactive notes with audio playback, flashcards, diagrams and full-text search.' },
  { icon: '${SPARK}',  title: 'DegreeAI 24/7 Study Coach',  desc: 'Solve exam questions instantly, generate mock tests and get step-by-step explanations.' },
  { icon: '${GROUP}',  title: '1-on-1 Peer Tutoring',        desc: 'Book private sessions with verified top seniors from UNILAG, UNIBEN, OAU, ABU and UI.' },
];
const FE = [
  { icon: '${DOC}',    title: 'Sell Your Lecture Notes',  desc: 'Upload study packs, set your Naira price and earn on every download automatically.' },
  { icon: '${TARGET}', title: 'Become a Paid Tutor',      desc: 'Set your rate (N1,500 to N5,000 per hour) and tutor junior students through the platform.' },
  { icon: '${CHART}',  title: 'Affiliate Commissions',    desc: 'Earn 50-60% weekly commissions recommending Selar digital courses to classmates.' },
  { icon: '${BULB}',   title: 'Digital Skills Tracks',    desc: 'Learn AI content creation, faceless YouTube and remote freelancing, then monetise immediately.' },
];
const FAQS = [
        ...
        <div style={{ maxWidth:'840px', margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(300px,1fr))', gap:'1.25rem', marginBottom:'2.5rem' }}>
            ...
            <div style={{ background:'var(--surface-variant)', border:'1px solid rgba(59,130,246,0.22)', borderRadius:'14px', padding:'1.75rem' }}>
              <div style={{ width:40, height:40, borderRadius:'8px', background:'rgba(59,130,246,0.1)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.25rem', marginBottom:'1rem' }}>${GRAD}</div>
            ...
            <div style={{ background:'var(--surface-variant)', border:'1px solid rgba(201,150,62,0.22)', borderRadius:'14px', padding:'1.75rem' }}>
              <div style={{ width:40, height:40, borderRadius:'8px', background:'rgba(201,150,62,0.1)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.25rem', marginBottom:'1rem' }}>${MONEY}</div>
