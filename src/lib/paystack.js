...
const PAYSTACK_PUBLIC_KEY = process.env.REACT_APP_PAYSTACK_PUBLIC_KEY || 'pk_live_90b0c934f9a5e9ff9868eae0207cec662e5ba03d';
...
export const payForProSubscription = async ({ user, onSuccess, onClose }) => {
  ...
  const rawPlan = process.env.REACT_APP_PAYSTACK_PRO_PLAN_CODE;
  const validPlan = rawPlan && !rawPlan.includes('your_plan_code') && rawPlan.startsWith('PLP_') ? rawPlan : undefined;
...
const verifyAndActivate = async ({ reference, type, resourceId, userId, bookingId, tutorUserId, amount }) => {
  if (type === 'resource') {
    ...
    if (rpcErr || !success) {
      console.warn('[Paystack] RPC confirm failed, falling back to direct table update...');
