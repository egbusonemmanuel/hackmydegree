...
export const updateProfile = async (userId, updates) => {
  // Security Allowlist: prevent tampering with balance, is_admin, total_earned, etc.
...
export const uploadAvatar = async (userId, file) => {
  ...
  if (!allowedExtensions.includes(ext)) {
    return { error: new Error('Invalid image type. Please upload a JPG, PNG, or WebP image.') };
  }

  if (file.size > 5 * 1024 * 1024) {
    return { error: new Error('Avatar image must be under 5MB.') };
  }
